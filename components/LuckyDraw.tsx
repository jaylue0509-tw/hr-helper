...
import { Trophy, RotateCcw, Play, Square, Sparkles, PartyPopper, Zap, LayoutGrid, Aperture, Code2, Disc, MoveVertical, Trash2 } from 'lucide-react';
import { LuckyDrawModal } from './LuckyDrawModal';
import { syncWinnerToBackend, clearWinnersFromBackend } from '../backend';
...
export const LuckyDraw: React.FC<LuckyDrawProps> = ({ candidates }) => {
  ...
  const stopDraw = () => {
    ...
    // Sync winner to Google Sheets
    syncWinnerToBackend(winner);
  ...
  const resetHistory = async () => {
    if (!window.confirm("確定要清除所有中獎紀錄嗎？這也會同步清除試算表上的資料。")) return;
      ...
      <div className="lg:col-span-4 flex flex-col gap-6 order-2 lg:order-1">
        ...
        <Card className="flex-1 overflow-hidden flex flex-col min-h-[300px]">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold tracking-wide text-[#1D1D1F] drop-shadow-sm">中獎名單</h3>
            {history.length > 0 && (
              <button
